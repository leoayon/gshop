    ...
    <div class="info-content">
      <section class="section">
        ...
        <div class="delivery">
          ...
          <div class="delivery-money">配送费￥{{shopInfo.deliveryTime}}</div>
        </div>
      </section>
      <div class="split"></div>
      <section class="section">
...
<script>
  ...
  export default {
    mounted () {
      /*
        如果shopInfo有数据，则进行初始化滚动；
        如果没数据，则监视(watch)shopInfo，直到shopInfo有数据才去初始化滚动
        (从其他两个子组件切换过来的时候，shopInfo有数据)
        (直接刷新当前页面，则shopInfo无数据)
        shopInfo是在shop组件中mounted中异步获取的
      */
    ...
    methods: {
      ...
      _initScroll () {
        ...
        const picUl = this.$refs.picUl
        const liWidth = 120
        const space = 6
        // 实时获取picUl的宽度
        picUl.style.width = (liWidth + space) * count - space + 'px'
        new BScroll('.shop-info', {
          click: true
        })
        new BScroll('.pic-wrapper', {
          click: true,
          // scrollX默认值为false, scrollY默认值为true
          scrollX: true
        })
      }
    },
